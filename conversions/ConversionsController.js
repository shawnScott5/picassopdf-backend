    ...
    async renderHTMLToPDF(html, css, options = {}) {
        try {
            ...
            await page.setViewport({ 
                width: 1200, 
                height: 1600,  // Taller for better page calculations
                deviceScaleFactor: 1
            });

            // Get professional page break CSS
            const professionalPageBreakCSS = this.getProfessionalPageBreakCSS();
            
            // Combine all CSS with professional page breaks taking precedence
            const combinedCSS = `${css}\n\n${professionalPageBreakCSS}`;

            // Enhanced HTML structure with better semantic markup
            const fullHTML = `
                <!DOCTYPE html>
                <html lang="en">
                <head>
                    ...
                    <title>Professional PDF Document</title>
            ...
            // Analyze and optimize content for professional page breaks
            await this.analyzeAndOptimizeContent(html, page);

            // Professional PDF generation with enhanced settings
            const pdfOptions = {
                format: options.format || 'A4',
                printBackground: true,
                margin: options.margin || {
                ...
                displayHeaderFooter: options.displayHeaderFooter || false,
                preferCSSPageSize: true,
                // Enhanced settings for professional output
            ...
            // Add header/footer if requested
            if (options.displayHeaderFooter) {
                pdfOptions.headerTemplate = options.headerTemplate || '<div style="font-size: 10px; text-align: center; width: 100%;"><span class="title"></span></div>';
                pdfOptions.footerTemplate = options.footerTemplate || '<div style="font-size: 10px; text-align: center; width: 100%;"><span class="pageNumber"></span> / <span class="totalPages"></span></div>';
                pdfOptions.margin.top = '20mm';
                pdfOptions.margin.bottom = '20mm';
            }
