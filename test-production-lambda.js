// Test script to verify Lambda connection from production
import LambdaService from './services/LambdaService.js';
import dotenv from 'dotenv';

dotenv.config();

async function testProductionLambda() {
    console.log('🧪 Testing Lambda connection from production...');
    ...
    try {
        ...
        if (isAvailable) {
            ...
            const testHtml = `
                ...
                <html>
                <head>
                    <title>Test PDF from Production</title>
                    <style>
                ...
                <body>
                    <h1>Hello from Production + Lambda!</h1>
            ...
            const pdfBuffer = await lambdaService.convertToPDF(testHtml, {
                format: 'A4',
                filename: 'test-from-production.pdf'
            });
            ...
            console.log('🔧 Check your AWS credentials and region settings');
        }
